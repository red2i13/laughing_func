...
int neighbors(node **queue, node *cur,char *map[], int vis[row][col])
{
    int x = cur->x;
    int y = cur->y;
    //printf("(((x: %i y: %i vis[x-1][y] = %i grid[x-1][y] = %i)))\n", x-1, y, vis[x-1][y], grid[x-1][y]);
    if(x + 1 < row - 1 && map[x + 1][y] != '1' && vis[x + 1][y] == 0)
    {
        enqueue(queue, x + 1, y);
    }
    if(x - 1 > 0 && map[x - 1][y] != '1' && vis[x - 1][y] == 0)
    {
        enqueue(queue, x - 1, y);
    }
    if(y + 1 < col-1 && map[x][y + 1]!= '1' && vis[x][y + 1] == 0)
    {
        enqueue(queue, x, y + 1);
        //printf("yeeees\n");
    }
    if(y - 1 > 0 && map[x][y - 1]!= '1' && vis[x][y - 1] == 0)
    {
        enqueue(queue, x, y - 1);
    }
    return(EXIT_SUCCESS);
...
//old func
int bfs(char *map[] ,int st_x, int st_y)
{
    int arr[row][col]; 
